    ...
    for i in range(0, 107):
        ...
        try:
            ...
            while True and len(exported_data) <= 1487:
                ...
                control.gear = 3
                control.brake = 0.0
                if len(list_theta_e) == 0:
                    past_location = world.player.get_transform()
                    past_time = -1
                    past_v = 0
                else:
                ...
                v_est = np.sqrt((location.location.x - past_location.location.x) ** 2 +
                                (location.location.y - past_location.location.y) ** 2) / (time - past_time)
                d = -_ie
                d_est_1 = (- (v_est - past_v) / (time - past_time) - 1.0954 *
                           v_est - 0.007 * v_est * v_est + 3.7387) / 0.1521
                d_est_2 += (3.6 * v_est - 20) * (time - past_time)
                ...
                while diff_theta_waypoint < -1.5:
                    diff_theta_waypoint += PI
                dot_phi_t = diff_theta_waypoint * v_est / \
                ...
                steering_wheel_input, h, h_dire, feasible = safe_ctrl(
                    jnp.array([cte, v_est, theta_e, d]), dot_phi_t)
